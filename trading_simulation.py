...
# ========== VR FUNCTIONS ==========
# Replace show_vr_mode() with this browser-based implementation
def show_vr_mode():
    """WebXR-compatible VR experience using Three.js"""
    st.markdown("""
    ## 🧙‍♂️ Immersive Quidditch VR
    <div id="vr-container" style="width:100%; height:500px; border:2px solid #D4AF37; border-radius:10px;"></div>
    ...
    // Create pitch
    const pitchGeometry = new THREE.PlaneGeometry(100, 70);
    ...
    // Add seekers
    const houseData = """ + json.dumps([
        {
            "name": house,
            "color": HOUSES[house]['color'],
            "position": st.session_state.game['positions'][house]
        } 
        for house in HOUSES
    ]) + """;
    ...
    // Add snitch if active
    """ + ("""
    if (true) {  // Snitch is active
        ...
        snitch.position.set(
            """ + str(st.session_state.game['snitch_position'][0]*25) + """,
            10,
            """ + str(st.session_state.game['snitch_position'][1]*25) + """
        );
        scene.add(snitch);
    }
    """ if st.session_state.game['snitch'] else "") + """
    ...
    vrButton.style.background = 'linear-gradient(#D4AF37, #F0E68C)';
    ...
    <div style="background:rgba(0,0,0,0.7); padding:15px; border-radius:10px; margin-top:20px;">
        <h4>VR Controls</h4>
        <p>🖱️ <strong>Desktop:</strong> Left-click+drag to rotate | Right-click+drag to pan</p>
        <p>📱 <strong>Mobile:</strong> Touch+move to rotate | Two-finger pan</p>
        <p>🕶️ <strong>VR Headsets:</strong> Click "ENTER VR" button</p>
    </div>
    """, unsafe_allow_html=True)
